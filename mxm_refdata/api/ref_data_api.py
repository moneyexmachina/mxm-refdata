...
class RefDataAPI:
    ...
    def get_contracts_for_product(self, product_id: str) -> List[FuturesContract]:
        ...
        with self.session_manager.db_session_scope() as session:
            ...
            result = [orm_to_obj(contract) for contract in contracts]
        period_ids = {c.period_id for c in result}
        if period_ids:
            periods = (
            ...
            period_by_id = {p.period_id: orm_to_obj(p) for p in periods}

            # Deterministic ordering: by Period (as defined in Period.__lt__), then contract_id
            result.sort(key=lambda c: (period_by_id[c.period_id], c.contract_id))
        self.cache.set(cache_key, result)
        return result
